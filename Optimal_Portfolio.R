...
OptimalPortfolio <- function(Expt_Ret_Matr, covar_matrix, Rf, short) {
  ...
  if(short == "yes"){
    ...
    A_mat <- cbind((Expt_Ret - Risk_Free), diag(1, nrow(cov_matr)))
...
getData <- function(..., start, end) {
  ...
  for(i in 1 : length(arguments[[1]])){
    ...
    stocks_data[[i]] <- Quandl(arguments[[1]][i], collapse = "monthly", start_date = start, end_date = end)
  }
  return(stocks_data)
}
...
getYearlyReturns <- function(list) {
  ...
  for(i in 1 : length(list)){
    stock_returns <- list()
    if (nrow(list[[i]]) <= 12){
      stock_returns <- append(stock_returns, (list[[i]][1, 5] - list[[i]][nrow(list[[i]]), 5]) / list[[i]][nrow(list[[i]]), 5])
    }
...
getEffFrontier <- function(Min_Weights, Effi_weights, Exp_Ret, cov_matrix, short) {
